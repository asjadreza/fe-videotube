...
const Header = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Retrieve access token from localStorage or Cookies
    const tokenFromLocalStorage = localStorage.getItem("accessToken");
    const tokenFromCookies = Cookies.get("accessToken");

    if (tokenFromLocalStorage) {
      setAccessToken(tokenFromLocalStorage); // Set access token from localStorage
    } else if (tokenFromCookies) {
      setAccessToken(tokenFromCookies); // Set access token from cookies
    }

    // Listen for localStorage changes (like after login)
    const handleStorageChange = () => {
      ...
      // Check for changes in access token
